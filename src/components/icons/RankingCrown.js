import React from "react";

const RankingCrown = ({ height, width }) => {
  return (
    <div>
      <svg
        height= {height ? height + 'px' : '30px'}
        width= {width ? width + 'px' : '30px'}
        version="1.1"
        id="_x35_"
        viewBox="0 0 512 512"
      >
        <g>
          <path
            fill="#856330"
            d="M480.928,399.689V162.177C498.525,159.334,512,144.189,512,125.782
		c0-20.433-16.56-36.993-36.985-36.993c-20.44,0-37,16.56-37,36.993c0,11.684,5.522,21.982,13.986,28.755l-65.783,82.222
		L275.312,90.566c16.517-7.398,28.038-23.943,28.038-43.218C303.35,21.201,282.156,0,256.003,0
		c-26.154,0-47.349,21.201-47.349,47.347c0,19.275,11.52,35.82,28.038,43.218l-110.91,146.193l-65.776-82.222
		c8.461-6.773,13.987-17.071,13.987-28.755c0-20.433-16.563-36.993-37.003-36.993C16.56,88.789,0,105.349,0,125.782
		c0,18.407,13.479,33.552,31.079,36.396v237.512C17.062,400.471,5.92,411.978,5.92,426.177c0,14.719,11.919,26.638,26.641,26.638
		h60.67h139.089h247.128c14.712,0,26.638-11.919,26.638-26.638C506.087,411.97,494.936,400.471,480.928,399.689z"
          />
          <g>
            <g>
              <g>
                <polygon
                  fill="#F6E6A6"
                  points="256.003,65.108 125.782,236.759 31.079,118.383 31.079,423.214 256.003,423.214 				"
                />
                <polygon
                  fill="#E8C581"
                  points="256.003,65.108 386.218,236.759 480.928,118.383 480.928,423.214 256.003,423.214 				
					"
                />
              </g>
              <g>
                <polygon
                  opacity="0.13"
                  fill="#C69950"
                  points="409.899,407.436 283.62,281.15 228.384,324.56 317.166,413.356 				"
                />
                <path
                  fill="#70B3AE"
                  d="M256.003,266.36c19.615,0,35.506,15.899,35.506,35.514c0,19.615-15.891,35.521-35.506,35.521
					V266.36z"
                />
                <path
                  fill="#96D3D5"
                  d="M256.003,337.395c-19.616,0-35.508-15.905-35.508-35.521c0-19.616,15.891-35.514,35.508-35.514
					V337.395z"
                />
              </g>
            </g>
            <g>
              <path
                fill="#E8C581"
                d="M256.003,0c26.154,0,47.347,21.201,47.347,47.347c0,26.162-21.193,47.362-47.347,47.362V0z"
              />
              <path
                fill="#F6E6A6"
                d="M256.003,94.709c-26.154,0-47.349-21.2-47.349-47.362C208.654,21.201,229.848,0,256.003,0V94.709z
				"
              />
            </g>
            <g>
              <path
                fill="#F6E6A6"
                d="M73.992,125.782c0,20.433-16.563,36.992-37.003,36.992c-20.43,0-36.99-16.56-36.99-36.992
				s16.56-36.993,36.99-36.993C57.429,88.789,73.992,105.349,73.992,125.782z"
              />
              <path
                fill="#E8C581"
                d="M512,125.782c0,20.433-16.56,36.992-36.985,36.992c-20.44,0-37-16.56-37-36.992
				s16.56-36.993,37-36.993C495.44,88.789,512,105.349,512,125.782z"
              />
            </g>
            <g>
              <path
                fill="#715022"
                d="M506.087,426.177c0,14.719-11.926,26.638-26.638,26.638H32.562
				c-14.723,0-26.641-11.919-26.641-26.638l0,0c0-14.711,11.919-26.637,26.641-26.637h446.887
				C494.161,399.54,506.087,411.466,506.087,426.177L506.087,426.177z"
              />
              <rect
                x="93.232"
                y="399.54"
                fill="#856330"
                width="139.089"
                height="53.275"
              />
            </g>
          </g>
        </g>
      </svg>
    </div>
  );
};

export default RankingCrown;
